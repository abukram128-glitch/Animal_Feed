...
def send_code_to_mail(receiver_email):
    ...
    msg['Subject'] = "🌾 السورس كود الاحترافي الشامل - منصة تاور V3"
...
# مصفوفة الإنتاج، السلالات، ونسب البروتين الحتمية برمجياً
SECTOR_OPTIONS_MAP = {
    "الأبقار وسلالاتها": {
...
# =====================================================================
# 4. بورصة أسعار الخامات والحيوانات الحية للمدن والولايات (حسابات دقيقة)
# =====================================================================
LIVE_FEED_PRICES = {
...
with tabs[0]:
    ...
    # ==========================================
    # 📋 نظام خانات البروتين المزدوج (البرمجية والاختيارية)
    # ==========================================
    ...
    # 1. الخانة البرمجية الحتمية والمثبتة علمياً
    ...
    # 2. الخانة الاختيارية الحرة للمستشار
    with col_cp_o:
    ...
    if st.button("🚀 تشغيل محرك التركيب والاحتساب البرمجي الصارم للبروتين والخلطة", type="primary", use_container_width=True):
        ...
        # حساب حاصل بروتين المزيج برمجياً بدقة وعزله في متغير مستقل ليطابق الفسيولوجيا
        calculated_cp = 0.0
        ...
        with res_col1:
            ...
            ton_cost = sum([(v/100) * ingredient_prices.get(k, 300.0) for k, v in formula_results.items() if k in ingredient_prices])
...
# ==========================================
# 🏷️ 7. مصمم بطاقات الديباجة والتسويق النهائي الشامل للصورة
# ==========================================
if st.session_state["user_role"] == "admin" and st.session_state["active_formula"]:
    ...
    # مصفوفة الصور التخطيطية التفصيلية حسب قطاع العلف الفعلي المختبر
