...
# 5. إضافة صفة اختصاصي تغذية الحيوان
...
def voice_guide_sequential(messages, lang="ar", delay_between=2.0):
    """تشغيل رسائل صوتية متسلسلة مع تأخير لمنع التداخل"""
    ...
    for i, msg in enumerate(messages):
        if msg:
            ...
            if audio_b64:
                ...
                # انتظار انتهاء الصوت (تقديري)
                word_count = len(msg.split())
                duration = max(2.0, word_count * 0.3 + 1.0)
                time.sleep(duration)
                # تأخير إضافي بين الجمل
                if i < len(messages) - 1:
                    time.sleep(1.0)
...
def play_full_guide_audio():
    """تشغيل الشرح الصوتي الكامل للمنصة بدون ازدواجية"""
    messages = [
...
class AuthManager:
    ...
    def _create_default_users(self):
        default_users = [
            ('admin', 'admin123', 'owner', 'الاختصاصي م. عبد القادر إسماعيل تاور - اختصاصي تغذية الحيوان', 'admin@tawornology.com', '+249123456789', 'تغذية حيوان', 10),
...
# =====================================================================
# مولد PDF المحسّن (مع اسم طالب العلف)
# =====================================================================
class ProfessionalPDFGenerator:
    ...
    def generate_comprehensive_report(self, formula, target_dp, breed, cost, city, local_cost, local_sym, computed_se, user_name, requester_name="", standard=None, include_charts=True, extra_info=None):
        ...
        for line in [f"👨‍💻 المشرف العام: {supervisor}", f"📌 الموقع الجغرافي: {city}", f"🐾 الفصيل المستهدف: {breed}", f"📅 تاريخ الإصدار: {datetime.now().strftime('%Y-%m-%d %H:%M')}"]:
            story.append(p(line))
        if requester_name:
    ...
    def generate_lab_report(self, analysis_results, animal_type, stage, user_name, standard=None, evaluation=None):
        ...
        supervisor = "الاختصاصي م. عبد القادر إسماعيل تاور - اختصاصي تغذية الحيوان"
        story.append(p(f"👨‍💻 المشرف العام: {supervisor}", 'subtitle'))
        story.append(Spacer(1, 5))
...
# =====================================================================
# مكتبة الأعلاف الموسعة (90+ مادة) - (مختصرة لتوفير المساحة)
# =====================================================================
BIG_FEEDS_LIBRARY = {
...
# =====================================================================
# دالة تركيب العلف المتقدم (مع اسم طالب العلف وتقييد 75%)
# =====================================================================
def render_feed_formulation(animal_key, display_name, icon, default_breeds, default_stages, default_dp, default_se, img_key, has_measurements=True):
    ...
    # حقل اسم طالب العلف
    requester_name = st.text_input("👤 اسم طالب العلف (المربي / المزرعة):", placeholder="أدخل اسم المربي أو المزرعة", key=f"{animal_key}_requester")
            ...
            if len(selected_ingredients) < 3:
                ...
                try:
                    ...
                    if res.success:
                        ...
                        # التحقق من المطابقة مع المعايير القياسية (نسبة 75% على الأقل)
                        ...
                        if standard:
                            if 'dp' in standard:
                                ...
                                if dp_ratio < 75:
                                    ...
                                    warnings_list.append(f"نسبة البروتين المهضوم {dp_ratio:.1f}% من المعيار (أقل من 75%)")
                            if 'se' in standard:
                                se_ratio = computed_se_total / standard['se'] * 100 if standard['se'] > 0 else 0
                                if se_ratio < 75:
                                    meets_standard = False
                                    warnings_list.append(f"نسبة معادل النشاء {se_ratio:.1f}% من المعيار (أقل من 75%)")
                            if 'cp' in standard:
                                cp_calc = actual_dp_target / 0.80
                                cp_ratio = cp_calc / standard['cp'] * 100 if standard['cp'] > 0 else 0
                                if cp_ratio < 75:
                                    meets_standard = False
                                    warnings_list.append(f"نسبة البروتين الخام {cp_ratio:.1f}% من المعيار (أقل من 75%)")
                        else:
                            st.warning("⚠️ لا توجد معايير قياسية لهذه المرحلة، سيتم قبول الخلطة بدون تقييد.")
                        
                        if not meets_standard:
                            st.error("❌ الخلطة لا تلبي الحد الأدنى من المعايير القياسية (75%). يرجى تعديل المكونات.")
...
if not st.session_state["approved"]:
    ...
    st.markdown("<p style='text-align:center; color:#888; font-size:0.9rem;'>الإصدار المتكامل 16.0 - مع تقييد 75% من المعايير</p>", unsafe_allow_html=True)
...
if st.session_state["user_role"] == "owner" and len(tabs) > 14:
    with tabs[14]:
        ...
        st.info("هذه الخاصية متاحة فقط للمالك (الاختصاصي م. عبد القادر إسماعيل تاور - اختصاصي تغذية الحيوان)")
