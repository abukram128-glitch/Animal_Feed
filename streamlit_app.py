# ============================================================================
# تاور نولجي Tawornology العلمية - الإصدار المتكامل النهائي 12.1
# ============================================================================
...
# ============================================================================
# هذا الإصدار هو الكود الكامل غير المختصر، ويحتوي على جميع الوظائف:
# - نظام الصوت (يعمل عند الضغط على الأزرار)
# - قاعدة بيانات SQLite متكاملة (13 جدولاً)
# - تركيب الأعلاف لجميع الحيوانات (أبقار، أغنام، ماعز، خيول، إبل، دواجن، أسماك)
# - المختبر المتقدم مع تقرير PDF مطابق للصور
# - إدارة المزارع والدورات الإنتاجية
# - بورصة الأسعار، المستودعات، الفواتير، الإنتاج اليومي، التقارير، التنبيهات
# - المراجع العلمية، المساعدة، دليل المستخدم، وإرسال الكود
# - توقيع المشرف (الاختصاصي م. عبد القادر إسماعيل تاور) في جميع تقارير PDF
# - تصميم عصري مع شريط دعاء متحرك
...
# =====================================================================
# قاعدة البيانات المتقدمة (SQLite) - كاملة غير مختصرة
# =====================================================================
...
# =====================================================================
# نظام إدارة المزارع المتقدم (كامل)
# =====================================================================
...
# =====================================================================
# مولد PDF (مع دعم العربية والألوان والرموز وتوقيع المشرف - مطابق للصور)
# =====================================================================
class ProfessionalPDFGenerator:
    ...
    def generate_lab_report(self, analysis_results, animal_type, stage, user_name, standard=None, evaluation=None):
        """
        توليد تقرير مختبر مطابق تماماً للصور المعروضة في التطبيق.
        يتضمن: جدول المكونات، النتائج المحسوبة، مقارنة مع المعايير، التقييم، والمخطط البياني.
        """
        buffer = io.BytesIO()
        ...
        if analysis_results:
            ...
            # 3. المقارنة مع المعايير القياسية (إن وجدت)
            if standard:
            ...
            # 5. المخطط البياني (المقارنة) - مطابق للصورة
...
if not st.session_state["approved"]:
    ...
    st.markdown("<p style='text-align:center; color:#888; font-size:0.9rem;'>الإصدار المتكامل 12.1 - النظام الشامل النهائي</p>", unsafe_allow_html=True)
    ...
    with animal_tabs[7]:
        ...
        if st.button("🧪 تشغيل التحليل المخبري", type="primary", use_container_width=True):
            ...
            if total <= 0:
                ...
                # تحميل تقرير المختبر PDF (مطابق للصور)
                try:
...
# =====================================================================
# التبويب 1: إدارة المزارع (جميع المستخدمين يمكنهم العرض، ولكن الإضافة للمختصين فقط)
# =====================================================================
with tabs[1]:
