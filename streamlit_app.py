...
• نظام مراجع علمية ومساعدة ذكية.
• 13 ميزة متقدمة تشمل التوصيات الذكية، التحليل المالي، البصمة الكربونية وغيرها.
...
welcome_messages = {
    "owner": {"bg": "#eff6ff", "border": "#1d4ed8", "text": "👑 أهلاً بك في منصتك، الاختصاصي م. عبد القادر إسماعيل تاور. نظام التوازن الدقيق بالبروتين المهضوم ومعادل النشاء قيد التشغيل الآن بكفاءة متناهية. كما تم تفعيل إدارة مزارع الدجاج اللاحم وجميع الميزات المتقدمة."},
...
# ==========================================
# 14. التبويب الأول: النمذجة والحسابات العلفية (الكامل مع المختبر وتقدير الوزن)
# ==========================================
with tabs[0]:
    ...
    # -------------------------------------------------------------------------
    # التبويب الفرعي: مختبر التحليل (المختبر)
    # -------------------------------------------------------------------------
...
if "💡 المساعدة الذكية" in tabs_titles:
    ...
    with tabs[help_tab_index]:
        ...
        quick_questions = ["ما هو البروتين المهضوم", "ما هو معادل النشاء", "ما هو مؤشر EPEF", "كيف يتم تركيب العلف الأمثل"]
...
# ==========================================
# 24. تبويب الميزات المتقدمة (التوصيات 1-13)
# ==========================================
if "🚀 الميزات المتقدمة (13 توصية)" in tabs_titles:
    with tabs[tabs_titles.index("🚀 الميزات المتقدمة (13 توصية)")]:
                ...
                if st.button("🔍 البحث عن بدائل", key="find_alternatives"):
                    ...
                    if alternatives:
                        for alt in alternatives:
                            st.markdown(f"• {alt['المادة']} - توفير {alt['التوفير_المتوقع']} - الملاءمة: {alt['الملاءمة']}")
                    else:
        ...
        with st.expander("📊 2. نظام التقارير المتقدمة", expanded=False):
            ...
            if st.button("📥 تصدير التقرير", key="export_report"):
                report_gen = AdvancedReportGenerator()
                formulas = [st.session_state.get("active_formula", {})]
                names = ["خلطتي الحالية"]
                df = report_gen.generate_comparison_report(formulas, names)
                st.success("✅ تم تجهيز التقرير للتصدير")
                st.dataframe(df)
        ...
        with st.expander("🔄 7. التكامل مع الأنظمة الخارجية", expanded=False):
            ...
            with col_int2:
                if st.button("🔄 مزامنة ERP", key="sync_erp"):
                    ...
                    st.success(f"✅ تمت المزامنة")
        
        # ==========================================
        # 8. نظام التدريب والتعلم
        # ==========================================
        with st.expander("📚 8. نظام التدريب والتعلم", expanded=False):
            ...
            if st.button("🎓 تسجيل إكمال دورة", key="complete_course"):
                st.success("✅ تم تسجيل إكمال الدورة بنجاح!")
        ...
        with st.expander("🏥 10. نظام الاستشارات البيطرية", expanded=False):
            ...
            if st.button("🔍 تشخيص", key="diagnose"):
                ...
                if result.get('status') != 'not_diagnosed':
                    st.warning(f"⚠️ التشخيص المحتمل: {result.get('possible_causes', ['غير معروف'])[0]}")
                    st.info(f"التوصية: {result.get('recommendation', 'استشارة طبيب بيطري')}")
        
        # ==========================================
        # 11. منصة المجتمع الحيواني
        # ==========================================
        with st.expander("💬 11. منصة المجتمع الحيواني", expanded=False):
            ...
            with tab_questions:
                ...
                if st.button("❓ طرح سؤال", key="ask_question"):
                    if question:
                        st.success(f"✅ تم طرح السؤال: {question}")
                    else:
            ...
            with tab_stories:
                ...
                if st.button("⭐ مشاركة قصة نجاح", key="share_story"):
                    if title and story:
                        st.success(f"✅ تم مشاركة قصة النجاح: {title}")
                    else:
                        st.warning("⚠️ يرجى إدخال العنوان والقصة")
        
        # ==========================================
        # 12. نظام المؤشرات الحيوية
        # ==========================================
...
with tabs[guide_tab_index]:
    ...
    with col_guide:
        ...
        <div class="book-chapter">📌 إدارة مزارع الدجاج اللاحم (خاص بالمالك)</div><div class="book-body">• تسجيل بيانات الدورة اليومية (العمر، العدد، الأوزان، الاستهلاك، النافق، المستبعدين، الظروف البيئية).<br>• حساب تلقائي لمؤشرات ADG، FCR، EPEF، ونسب النفوق والاستبعاد.<br>• جدول الحرارة والرطوبة المرجعي حسب العمر.<br>• تقرير يومي شامل يمكن مشاركته عبر واتساب.</div>
        <div class="book-chapter">📌 الميزات المتقدمة (13 توصية)</div><div class="book-body">• <b>نظام التوصيات الذكي:</b> اقتراح بدائل للمواد العلفية.<br>• <b>نظام التقارير المتقدمة:</b> تقارير مقارنة وتصدير متعدد الصيغ.<br>• <b>نظام الإشعارات:</b> إشعارات متعددة القنوات.<br>• <b>معمل التحليل:</b> تحليل الجودة وتقدير القيم الغذائية.<br>• <b>التحليل المالي:</b> حساب ROI ونقطة التعادل.<br>• <b>نظام الجودة:</b> التحقق من المطابقة للمواصفات.<br>• <b>التكامل مع الأنظمة:</b> ربط IoT ومزامنة ERP.<br>• <b>نظام التدريب:</b> دورات تدريبية وشهادات.<br>• <b>التطبيقات الجوالة:</b> رموز QR وإشعارات فورية.<br>• <b>الاستشارات البيطرية:</b> تشخيص الأمراض وخطط علاجية.<br>• <b>منصة المجتمع:</b> مناقشات وأسئلة وقصص نجاح.<br>• <b>المؤشرات الحيوية:</b> تحليل الصحة والتنبؤ بالأمراض.<br>• <b>البصمة الكربونية:</b> حساب الانبعاثات وتوصيات الاستدامة.</div>
        <div class="book-chapter">📌 خطوات تشغيل المنصة</div><div class="book-body"><b>الخطوة 1:</b> حدد القطاع والنوع الإنتاجي.<br><b>الخطوة 2:</b> اختر الخامات المتوفرة وأسعار السوق.<br><b>الخطوة 3:</b> اضغط على زر التشغيل للحصول على الخلطة المثلى.<br><b>الخطوة 4:</b> استعرض التقرير وقم بطباعة الديباجة أو تصدير PDF.<br><b>الخطوة 5 (للمالك):</b> استخدم تبويب إدارة الدجاج اللاحم لتسجيل ومتابعة أداء دورات التسمين.<br><b>الخطوة 6:</b> استكشف الميزات المتقدمة للحصول على تحليلات أعمق.</div></div>""", unsafe_allow_html=True)
    with col_actions:
        ...
        share_text_base = "أستخدم الآن منصة تاور العلمية للانتاج الحيواني وتركيب الاعلاف لحساب العلائق بأقل تكلفة ودقة علمية عالية، تحت إشراف م. عبد القادر إسماعيل تاور. تتضمن 13 ميزة متقدمة!"
