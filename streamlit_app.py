...
# ==========================================
# 3. إعدادات المنصة الرسمية والمظهر الفخم
# ==========================================
st.set_page_config(
...
# الأكواد المعتمدة (للتوافق مع الإصدار السابق)
def generate_secure_hash(code: str, salt: str = None) -> str:
    if salt is None:
        salt = secrets.token_hex(16)
    return hashlib.pbkdf2_hmac('sha256', code.encode(), salt.encode(), 100000).hexdigest()
...
- نظام قاعدة بيانات محلية SQLite
- نظام مصادقة متقدم
...
# ==========================================
# 5. كلاس إدارة مزارع الدجاج اللاحم (مطور)
# ==========================================
...
# ==========================================
# 8. دوال مساعدة للتنبيهات عبر واتساب
# ==========================================
...
# ==========================================
# 9. المكتبة والمتغيرات (نفس السابق)
# ==========================================
BIG_FEEDS_LIBRARY = {
...
if not st.session_state["approved"]:
    ...
    if login_option == "كود الدخول السري":
        ...
        # تسجيل الدخول بنظام المستخدمين الجديد
...
# ==========================================
# 13. الواجهة الرئيسية المطورة مع الشريط الجانبي
# ==========================================
# شريط جانبي للتنقل
with st.sidebar:
    ...
    role_names = {"owner": "👑 مالك", "specialist": "👨‍🔬 مختص", "breeder": "🌾 مربي"}
    st.markdown(f"""
    <div style='background: #f0fdf4; padding: 12px; border-radius: 10px; margin-bottom: 15px;'>
        <p style='margin: 0; font-weight: bold;'>{user_info.get('full_name', 'مستخدم')}</p>
        <p style='margin: 0; font-size: 0.8rem; color: #666;'>@{user_info.get('username', 'guest')}</p>
        <p style='margin: 0; font-size: 0.8rem;'><span style='background: #2e7d32; color: white; padding: 2px 10px; border-radius: 12px;'>{role_names.get(st.session_state.user_role, 'ضيف')}</span></p>
    </div>
    """, unsafe_allow_html=True)
    ...
    pages = {
        "🏠 الرئيسية": "dashboard",
        "🔬 تركيب الأعلاف": "formulator",
        "📊 بورصة الأسعار": "prices",
        "🏭 إدارة المخازن": "inventory",
        "🧾 الفواتير والمبيعات": "invoices",
        "🐔 مزارع الدجاج": "broiler",
        "📈 التحليلات": "analytics",
        "💬 التعليقات": "comments",
        "📖 الدليل": "guide"
    }
    
    # عرض الصفحات حسب الدور
    if st.session_state.user_role == "owner":
        display_pages = pages
    elif st.session_state.user_role == "specialist":
        display_pages = {k: v for k, v in pages.items() if v not in ["invoices", "broiler"]}
    else:  # breeder
        display_pages = {k: v for k, v in pages.items() if v in ["dashboard", "formulator", "guide"]}
    
    selection = st.radio("التنقل", list(display_pages.keys()))
    st.session_state.current_page = display_pages[selection]
...
with col_logo:
    if img_base64:
        st.markdown(f'<img src="data:image/jpeg;base64,{img_base64}" class="profile-img-style pulse-animation" style="width:80px; height:80px;">', unsafe_allow_html=True)
    else:
        st.markdown(f'<img src="{ANIMAL_IMAGES_RESOURCES["عام"]}" class="profile-img-style" style="width:80px; height:80px;">', unsafe_allow_html=True)
with col_title:
    ...
    st.markdown("<p style='color: #1565C0; text-align:right; font-size:1.1rem; margin-top:5px;'>محرك الاستمثال الخطي المتقدم | إدارة شاملة للإنتاج الحيواني</p>", unsafe_allow_html=True)
...
if current_page == 'dashboard':
    st.markdown('<div class="section-title">🏠 لوحة التحكم الرئيسية</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="metric-card">
            <h3 style="color: #1b5e20;">📊 الدورات</h3>
            <h2 style="color: #2e7d32;">{}</h2>
            <p>دورة نشطة</p>
        </div>
        """.format(len(st.session_state.get("broiler_farms", {}))), unsafe_allow_html=True)
    with col2:
        st.markdown("""
        <div class="metric-card">
            <h3 style="color: #1565C0;">📈 EPEF</h3>
            <h2 style="color: #1976D2;">{:.0f}</h2>
            <p>متوسط المؤشر</p>
        </div>
        """.format(285 if st.session_state.user_role == "owner" else 0), unsafe_allow_html=True)
    with col3:
        st.markdown("""
        <div class="metric-card">
            <h3 style="color: #E65100;">💰 التوفير</h3>
            <h2 style="color: #F57C00;">{:.0f}%</h2>
            <p>مقارنة بالتقليدي</p>
        </div>
        """.format(18.5), unsafe_allow_html=True)
    with col4:
        st.markdown("""
        <div class="metric-card">
            <h3 style="color: #2E7D32;">⭐ التقييم</h3>
    ...
    if st.session_state.user_role == "owner" and st.session_state.get("broiler_farms"):
        for farm_name, farm_data in list(st.session_state["broiler_farms"].items())[:3]:
            current = farm_data.get("current_data", {})
            with st.expander(f"🐔 {farm_name} - {current.get('flock_age_days', 0)} يوم"):
                cols = st.columns(3)
                with cols[0]:
                    st.write(f"**الوزن:** {current.get('current_weight_kg', 0):.3f} كجم")
                    st.write(f"**العدد:** {current.get('initial_birds', 0)}")
                with cols[1]:
                    st.write(f"**FCR:** {current.get('fcr', 0):.2f}")
                    st.write(f"**ADG:** {current.get('adg', 0):.1f} جم")
                with cols[2]:
                    st.write(f"**النفوق:** {current.get('mortality_rate', 0):.1f}%")
                    st.write(f"**الحيوية:** {current.get('livability', 0):.1f}%")
    else:
        st.info("لا توجد نشاطات حديثة")

# صفحة تركيب الأعلاف (دمج الكود الأصلي)
elif current_page == 'formulator':
    st.markdown('<div class="section-title">🔬 تركيب علفة نموذجية (أقل تكلفة بالبروتين المهضوم)</div>', unsafe_allow_html=True)
    
    # دمج الكود الأصلي لتركيب الأعلاف
...
# صفحة مزارع الدجاج اللاحم (خاص بالمالك)
elif current_page == 'broiler' and st.session_state["user_role"] == "owner":
