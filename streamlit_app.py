...
class AdvancedAIModel:
    ...
    def predict_with_confidence(self, historical_data, prediction_days):
        """التنبؤ مع مستوى الثقة"""
        trend = self.analyze_trend(historical_data)
        
        predictions = []
        for i in range(1, prediction_days + 1):
            pred = historical_data[-1] + trend.get('slope', 0) * i
            predictions.append(max(0, pred))
        
        confidence = min(0.95, trend.get('strength', 0) * 1.2)
