...
SENDER_PASSWORD = "oynz rdli tsdy ekdq"     # ✅ رمز التطبيق الخاص بك
...
def send_code_to_mail(receiver_email):
    ...
    msg['Subject'] = "🌾 السورس كود الكامل (Linear Programming) - منصة تاور الذكية للأعلاف"
    
    body = "السلام عليكم م. عبد القادر،\n\nمرفق مع هذه الرسالة النسخة البرمجية الشاملة والمستقرة لمنصة تاور الذكية لعام 2026 القائمة على محرك الاستمثال الخطي لتقليل نسبة الخطأ إلى الصفر.\n\nتحياتي،\nالنظام التلقائي للمنصة."
    ...
    try:
        ...
        attachment.add_header('Content-Disposition', 'attachment', filename="tower_full_lp_platform.py")
...
# =====================================================================
# 3. الهيكل المركزي للمخازن وبورصة تاور العالمية الشاملة ($)
# =====================================================================
INITIAL_INVENTORY = {
...
if "active_formula" not in st.session_state: st.session_state["active_formula"] = {"ذرة صفراء": 60.0, "كسب فول صويا 44%": 35.0, "ملح الطعام": 0.5, "الحجر الجيري (بودرة بلاط)": 1.5, "فوسفات ثنائي الكالسيوم (DCP)": 1.0, "مركزات دواجن وسمان": 2.0}
...
with col_title:
    ...
    st.markdown("<p style='color: #1565C0; text-align:right; font-size:1.2rem; margin-top:5px; margin-bottom:0;'>محرك الاستمثال الخطي الحاكم (Least-Cost LP) لمنع نسبة الخطأ الحسابي والغذائي</p>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with col_state:
        if user_country == "السودان": chosen_state = st.selectbox("اختر الولاية السودانية كاملة:", list(SUDAN_GEOGRAPHY.keys()))
        elif user_country == "ليبيا": chosen_state = st.selectbox("اختر الإقليم الجغرافي:", ["المنطقة الشرقية", "المنطقة الغربية", "المنطقة الجنوبية"])
        else: chosen_state = st.selectbox("الإقليم الإداري:", ["القاهرة الكبرى", "الدلتا", "الصعيد"])

    with col_city:
        if user_country == "السودان": user_city = st.selectbox("اختر المدينة / المركز التابع للولاية:", SUDAN_GEOGRAPHY[chosen_state])
        ...
        else: user_city = st.selectbox("اختر المدينة المصرية:", ["طنطا", "المنصورة", "الفيوم"])
    ...
    with col_sec: main_sector = st.selectbox("اختر القطاع الإنتاجي الرئيسي:", ["الطيور والسمان", "الأبقار وسلالاتها", "الماعز وسلالاته", "الخيول والفروسية", "الأسماك والأحياء المائية"])
    ...
    if show_measurements:
        ...
        with col_l: b_length = st.number_input("📏 طول الجسم (سم):", value=130.0)
    ...
    with col_p2:
        ...
        final_target_cp = st.slider("حدّد نسبة البروتين المستهدفة فنيّاً (محدد قاطع):", 10.0, max_value=45.0, value=default_cp) if override_cp else default_cp

    st.markdown('<div class="section-title">🌾 خامساً: اختيار الخامات المتوفرة بالمستودع للتركيب الخطي</div>', unsafe_allow_html=True)
    selected_ingredients = []; ingredient_prices = {}
    
    for cat_name, items in BIG_FEEDS_LIBRARY.items():
        if cat_name != "المكونات الدقيقة والمركزات":
            with st.expander(f"📁 {cat_name}", expanded=True):
                sub_cols = st.columns(3)
                for idx, (ing_name, _) in enumerate(items.items()):
                    with sub_cols[idx % 3]:
                        is_def = True if "ذرة صفراء" in ing_name or "صويا" in ing_name or "نخالة" in ing_name else False
    ...
    # حجز وبناء الإضافات الفنية والمركزات الإجبارية التي يحتاجها الطن لسلامة الماشية والخلط
    fixed_additives = {
        "ملح الطعام": 0.5, "مضاد سموم فطرية": 0.2, "الحجر الجيري (بودرة بلاط)": 1.5, 
        "فوسفات ثنائي الكالسيوم (DCP)": 1.0
    }
    if chosen_concentrate:
        fixed_additives[chosen_concentrate] = 5.0 if "دواجن" in chosen_concentrate else (8.0 if "أسماك" in chosen_concentrate else 2.5)

    # إدراج الإضافات الثابتة في الخلطة تلقائياً لضمان سلامة الطن
    ...
    st.markdown("---")
    if st.button("🚀 تشغيل محرك الاستمثال الخطي ومنع نسبة الخطأ حَقلياً", type="primary", use_container_width=True):
        
        # --- بناء المحددات الرياضية لمحرك البرمجة الخطية (Linear Programming) ---
        # 1. معامل دالة الهدف الكلفة الأدنى Z = c_vector * X
        c_vector = [ingredient_prices[ing] for ing in selected_ingredients]
        
        # 2. حدود المكونات الفرعية (Bounds) وقفل الإضافات الدقيقة
        bounds = []
        for ing in selected_ingredients:
            if ing in fixed_additives:
                bounds.append((fixed_additives[ing], fixed_additives[ing])) # حجز وقفل صارم للنسبة الصغرى
            else:
                bounds.append((0.0, 100.0))

        # 3. محددات التساوي (Equalities Matrix): مجموع العليقة = 100% والبروتين المستهدف بدقة
        ...
        # 4. محددات عدم التساوي (Inequalities Constraints): قفل الطاقة والنشويات الإجمالي بين 60% و 65%
        energy_row_min = []
        energy_row_max = []
        for ing in selected_ingredients:
        ...
        b_ub = [-60.0, 65.0] # إجبار مجموع الحبوب ألا يقل عن 60% ولا يزيد عن 65%

        # استدعاء المعالج الخطي عالي الأداء من Scipy
        ...
        if res.success:
            ...
            # =========================================================================
            # 🧪 محرك فحص ومعالجة علل الإنزيمات وبث الإشعارات المنبثقة لمدة 30 ثانية
            # =========================================================================
            mandatory_warnings = []
            
            # حالة التحمض والبيكربونات للأبقار والمجترات عند ارتفاع كتلة الحبوب
            if main_sector in ["الأبقار وسلالاتها", "الماعز وسلالاته"]:
                formula_results["بيكربونات الصوديوم (الصودا)"] = 0.75
                mandatory_warnings.append("🚨 إضافة إلزامية - بيكربونات الصوديوم (الصودا): تم حقن 7.5 كجم/الطن آلياً لحماية الكرش من التحمض الناتجة عن نسبة طاقة محكمة بين 60-65%.")

            # حالة الفايتيز لتحرير الفسفور في قطاع الدواجن والأسماك
            if main_sector in ["الطيور والسمان", "الأسماك والأحياء المائية"]:
                formula_results["إنزيم الفايتيز الزامي (Phytase Super-D)"] = 0.05
                mandatory_warnings.append("🚨 إضافة إلزامية - إنزيم الفايتيز (Phytase): تم دمجه لكسر روابط الفايتيك وتحرير الفسفور النباتي بالكامل.")

            # حالة استخدام القمح أو الشعير في طيور الدواجن (إنزيم الـ NSP)
            if main_sector == "الطيور والسمان" and ("شعير مطحون" in formula_results or "قمح محلي مصنّع" in formula_results):
                formula_results["إنزيم الـ NSP (زيلاناز + بيتا جلوكاناز)"] = 0.08
                mandatory_warnings.append("⚠️ علة فنية معالجة: تم دمج إنزيم الـ NSP لرفع كفاءة هضم الحبوب البديلة ومنع اللزوجة المعوية بالكامل.")

            # بث الإشعارات البرمجية التلقائية على شكل Toast يختفي بعد 30 ثانية
            ...
            with res_col1:
                ...
                st.info(f"📊 إجمالي نسبة مصادر الطاقة المحققة رياضياً: **{total_energy_pct:.2f}%** (تقع تماماً في النطاق الصارم المطلوب 60% - 65%)")
                
                ton_cost = res.fun / 100.0 if hasattr(res, 'fun') else 290.0
            ...
            st.error("❌ تعذر إيجاد حل رياضي متزن تماماً ضمن المحددات الحالية. يرجى تفعيل وإتاحة خامات إضافية في القائمة (مثل كسب صويا 48% أو نخالة) لتوسيع نطاق الموازنة لمحرك scipy.")
...
if st.session_state["user_role"] == "admin":
    ...
    with tabs[4]:
        ...
        trade_brand = st.text_input("اسم البراند التجاري لإصدار الشهادة الفنية:", "مجموعة تاور لإنتاج الأعلاف ومصنعات الإنتاج الحيواني")
        st.markdown(f"""
        <div class="sack-tag">
