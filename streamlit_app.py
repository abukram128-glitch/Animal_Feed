...
# ==========================================
# 1. إعدادات المنصة الرسمية والمظهر الأصلي المستقر
# ==========================================
...
# 💡 توليد الباركود لضمان عدم توقف السيرفر وبدون مكتبات خارجية مكسورة
...
# هندسة تحسين الخطوط والمحاذاة للمظهر الأساسي النظيف
st.markdown(
    """
    <style>
...
if not st.session_state["approved"]:
    st.markdown('<div class="main-box" style="max-width: 450px; margin: 80px auto; direction: rtl;">', unsafe_allow_html=True)
...
# =====================================================================
# 3. بنك الخامات الشامل والأقسام المعتمدة
# =====================================================================
BIG_FEEDS_LIBRARY = {
    ...
    "🧪 قسم الإنزيمات ومضادات السموم والمعالجات الذكية": {
...
with col_logo:
    ...
    else: st.markdown('<div class="profile-img-style" style="background:#2e7d32; display:flex; align-items:center; justify-content:center; color:white; font-size:1.8rem;">🌾</div>', unsafe_allow_html=True)

with col_title:
    st.markdown("<h1 style='color: #1b5e20; text-align:right; margin-bottom:0; font-size:1.7rem;'>منصة تاور الذكية للإنتاج الحيواني وصناعة الأعلاف 🌾</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='color: #c62828; text-align:right; font-weight: bold; margin-top: 5px; font-size:1.2rem;'>الخبير المستشار / م. عبد القادر إسماعيل تاور</h3>", unsafe_allow_html=True)

st.markdown("<hr style='border-top: 2px solid #2e7d32; margin:15px 0;'>", unsafe_allow_html=True)

# التبويبات الفنية والإدارية الكاملة
...
with tabs[0]:
    ...
    # 📏 شريط القياس المعتمد لنسبة البروتين
    default_cp = 23.0 if "بادي" in prod_stage else (21.0 if "نامي" in prod_stage else 18.0)
    final_target_cp = st.slider("حدد نسبة البروتين المستهدفة فنيّاً عبر شريط القياس (%):", 10.0, 45.0, value=default_cp)
    ...
    if st.button("🚀 معالجة التركيبة والمحاكاة الرياضية المدمجة", type="primary", use_container_width=True):
        ...
        # إعادة الحساب والتوزيع الدقيق والمتوافق مع الصور
        ...
        if "شعير مطحون" in selected_ingredients or "قمح محلي مصنّع" in selected_ingredients:
            ...
            mandatory_warnings.append("⚠️ <b>معالجة الـ NSP المعوي:</b> تم إدراج إنزيم الزيلاناز لعلاج لزوجة الهضم الناتجة عن ألياف الشعير والقمح.")
        ...
        st.success("🎯 تم احتساب التوافق العلفي والإنزيمي بدقة.")
...
# ==========================================
# التبويبات الإدارية للمالك فقط (البورصة، الفواتير، والباركود)
# ==========================================
if st.session_state["user_role"] == "admin":
    with tabs[1]:
        st.markdown('<div class="section-title">📊 لوحة أسعار بورصة تاور المركزية العالمية</div>', unsafe_allow_html=True)
        st.info("شاشة البورصة ومتابعة الأسعار المركزية العالمية محدثة بشكل حي.")
        
    with tabs[2]:
        st.markdown('<div class="section-title">🏭 إدارة وحركات المستودعات الرقمية</div>', unsafe_allow_html=True)
        st.info("شاشة جرد المستودعات وحركات الوارد والمنصرف الفعلي.")
    ...
    with tabs[4]:
        ...
        st.markdown(f"""
        <div class="sack-tag">
            <h2 style="text-align: center; color: #1b5e20; margin-top:0; font-weight: bold; font-size:1.3rem;">🌾 مجموعة تاور لإنتاج الأعلاف ومصنعات الإنتاج الحيواني 🌾</h2>
            ...
            <div style="text-align: center;">
                <img src="{qr_image_url}" style="width: 140px; height: 140px; border: 2px solid #1b5e20; padding: 4px; background: white; border-radius: 6px;">
            </div>
        </div>
        """, unsafe_allow_html=True)
